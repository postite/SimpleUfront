...
class erazor_ScriptBuilder {
	...
	public function blockToString($block) {
		...
		case 0:{
			...
			$tmp1 = _hx_string_or_null($this->context) . ".add('";
			$tmp2 = str_replace("'", "\\'", $s);
			{
				$tmp3 = _hx_string_or_null($tmp1) . _hx_string_or_null($tmp2) . "');\x0A";
				$GLOBALS['%s']->pop();
				return $tmp3;
			}
		}break;
		case 1:{
			$tmp2 = _hx_string_or_null(_hx_deref($block)->params[0]) . "\x0A";
			$GLOBALS['%s']->pop();
			return $tmp2;
		}break;
		case 2:{
			$tmp2 = _hx_string_or_null($this->context) . ".unsafeAdd(" . _hx_string_or_null(_hx_deref($block)->params[0]) . ");\x0A";
			$GLOBALS['%s']->pop();
			return $tmp2;
		}break;
		}
