		...
		while(true) {
			try {
				...
				if(is_string($e = $_ex_)){
					...
					$tmp = _hx_substr($e, 0, 16);
					if($tmp !== "Missing argument") {
						php_Lib::rethrow($e);
					}
					$expected = $args->length + 1;
					$tmp1 = erazor_hscript_EnhancedInterp::$re->match($e);
					if($tmp1) {
						$tmp2 = erazor_hscript_EnhancedInterp::$re->matched(1);
						$expected = Std::parseInt($tmp2);
						unset($tmp2);
					}
					if($expected > 15) {
						throw new HException("invalid number of arguments");
					} else {
						$tmp3 = $expected < $args->length;
						if($tmp3) {
...
function erazor_hscript_EnhancedInterp_0(&$capturedLocals, &$fexpr, &$me, &$params, $args) {
	{
		...
		$me->locals = $me->duplicate($capturedLocals);
		{
			...
			while($_g1 < $_g) {
				$i = $_g1++;
				$tmp1 = $args[$i];
				$me->locals->set(_hx_array_get($params, $i)->name, _hx_anonymous(array("r" => $tmp1)));
				unset($tmp1,$i);
			}
		}
		$r = null;
