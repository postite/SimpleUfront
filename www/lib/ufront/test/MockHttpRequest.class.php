...
class ufront_test_MockHttpRequest extends ufront_web_context_HttpRequest {
	public function __construct($uri = null) { if(!php_Boot::$skip_constructor) {
		...
		$tmp = ufront_web_UserAgent::fromString("Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:43.0) Gecko/20100101 Firefox/43.0");
		$this->setUserAgent($tmp);
	...
	public function setIsMultipart($isMultipart) {
		...
		if($isMultipart) {
			...
			$tmp = strtolower("Content-Type");
			ufront_core__MultiValueMap_MultiValueMap_Impl_::set($this1, $tmp, "multipart/form-data; charset=UTF-8");
		} else {
			$this2 = $this->get_clientHeaders();
			$tmp1 = strtolower("Content-Type");
			ufront_core__MultiValueMap_MultiValueMap_Impl_::set($this2, $tmp1, "application/x-www-form-urlencoded; charset=UTF-8");
		}
		{
			$GLOBALS['%s']->pop();
			return $this;
