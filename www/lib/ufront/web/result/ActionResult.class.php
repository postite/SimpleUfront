...
class ufront_web_result_ActionResult {
	...
	static function transformUri($actionContext, $uri) {
		...
		$__hx__spos = $GLOBALS['%s']->length;
		$tmp = StringTools::startsWith($uri, "~/");
		if($tmp) {
			$tmp1 = _hx_substr($uri, 2, null);
			{
				$tmp2 = $actionContext->httpContext->generateUri($tmp1, null);
				$GLOBALS['%s']->pop();
				return $tmp2;
			}
		} else {
	...
	static function wrap($resultValue) {
		...
		if($resultValue === null) {
			...
			if($actionResultValue === null) {
				$tmp = Std::string($resultValue);
				$actionResultValue = new ufront_web_result_ContentResult($tmp, null);
			}
			{
				$GLOBALS['%s']->pop();
