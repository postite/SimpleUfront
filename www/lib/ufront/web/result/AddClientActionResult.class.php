...
class ufront_web_result_AddClientActionResult extends ufront_web_result_CallJavascriptResult implements ufront_web_result_WrappedResult{
	...
	public function executeResult($actionContext) {
		...
		{
			$tmp2 = tink_core__Future_Future_Impl_::_tryMap($tmp, array(new _hx_lambda(array(&$_gthis, &$actionContext), "ufront_web_result_AddClientActionResult_0"), 'execute'));
			$GLOBALS['%s']->pop();
			return $tmp2;
		}
		$GLOBALS['%s']->pop();
...
function ufront_web_result_AddClientActionResult_0(&$_gthis, &$actionContext, $n) {
	{
		...
		$tmp1 = $response->get_contentType();
		if($tmp1 === "text/html") {
			$className = $_gthis->action;
			$serializedData = haxe_Serializer::run($_gthis->actionData);
			$script = "<script type=\"text/javascript\">" . _hx_string_or_null(("ufExecuteSerializedAction( \"" . _hx_string_or_null($className) . "\", \"" . _hx_string_or_null($serializedData) . "\" )")) . "</script>";
			$tmp2 = $response->getBuffer();
			$newContent = ufront_web_result_CallJavascriptResult::insertScriptsBeforeBodyTag($tmp2, (new _hx_array(array($script))));
