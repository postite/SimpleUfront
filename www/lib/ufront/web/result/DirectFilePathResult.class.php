...
class ufront_web_result_DirectFilePathResult extends ufront_web_result_ActionResult {
	...
	public function executeResult($actionContext) {
		...
		$tmp1 = StringTools::startsWith($this->filePath, $scriptDir);
		if($tmp1) {
			$url = _hx_substr($this->filePath, strlen(haxe_io_Path::removeTrailingSlashes($scriptDir)), null);
			{
				$tmp2 = _hx_deref(new ufront_web_result_RedirectResult($url, true))->executeResult($actionContext);
				$GLOBALS['%s']->pop();
				return $tmp2;
			}
		} else {
			$result = new ufront_web_result_FilePathResult($this->filePath, null, null);
			$tmp2 = haxe_io_Path::withoutDirectory($this->filePath);
			$result->setContentTypeByFilename($tmp2);
			{
				$tmp3 = $result->executeResult($actionContext);
				$GLOBALS['%s']->pop();
				return $tmp3;
			}
		}
