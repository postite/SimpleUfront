...
class ufront_web_UserAgent {
	...
	static function extractVersion($searchString, $s) {
		...
		$re = new EReg("(\\d+)\\.(\\d+)[^ ();]*", "");
		$tmp = $index + strlen($searchString) + 1;
		$tmp1 = _hx_substr($s, $tmp, null);
		$tmp2 = !$re->match($tmp1);
		if($tmp2) {
			$GLOBALS['%s']->pop();
			return null;
		...
		$tmp3 = $re->matched(0);
		$tmp4 = $re->matched(1);
		$tmp5 = Std::parseInt($tmp4);
		$tmp6 = $re->matched(2);
		$tmp7 = Std::parseInt($tmp6);
		{
			$tmp8 = _hx_anonymous(array("version" => $tmp3, "majorVersion" => $tmp5, "minorVersion" => $tmp7));
			$GLOBALS['%s']->pop();
			return $tmp8;
		}
		$GLOBALS['%s']->pop();
	...
	static function searchString($data, $s) {
		...
		$__hx__spos = $GLOBALS['%s']->length;
		{
			...
			while($_g < $data->length) {
				$d = $data[$_g];
				++$_g;
				$tmp = _hx_index_of($s, $d->subString, null);
				if($tmp >= 0) {
					$tmp1 = null;
					if(_hx_field($d, "versionSearch") === null) {
						$tmp1 = $d->identity;
					} else {
						$tmp1 = $d->versionSearch;
					}
					{
						$tmp2 = _hx_anonymous(array("app" => $d->identity, "versionString" => $tmp1));
						$GLOBALS['%s']->pop();
						return $tmp2;
						unset($tmp2);
					}
