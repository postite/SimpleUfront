...
class ufront_web_context_HttpContext {
		...
		if($this->session === null) {
			try {
				...
				$e = $_ex_;
				{
					...
					$tmp3 = Std::string($e);
					$tmp4 = "Failed to load UFHttpSession: " . _hx_string_or_null($tmp3) . ". Using VoidSession instead.";
					$tmp5 = haxe_CallStack::exceptionStack();
					$tmp6 = haxe_CallStack::toString($tmp5);
					$this->messages->push(_hx_anonymous(array("msg" => _hx_string_or_null($tmp4) . _hx_string_or_null($tmp6), "pos" => _hx_anonymous(array("fileName" => "HttpContext.hx", "lineNumber" => 236, "className" => "ufront.web.context.HttpContext", "methodName" => "new")), "type" => ufront_log_MessageType::$MLog)));
				}
			}
		}
		if($this->session === null) {
		...
		if($this->auth === null) {
			try {
				...
				$e1 = $_ex_;
				{
					...
					$GLOBALS['%s']->push($GLOBALS['%e'][0]);
					$tmp7 = Std::string($e1);
					$tmp8 = "Failed to load UFAuthHandler: " . _hx_string_or_null($tmp7) . ". Using NobodyAuthHandler instead.";
					$tmp9 = haxe_CallStack::exceptionStack();
					$tmp10 = haxe_CallStack::toString($tmp9);
					$this->messages->push(_hx_anonymous(array("msg" => _hx_string_or_null($tmp8) . _hx_string_or_null($tmp10), "pos" => _hx_anonymous(array("fileName" => "HttpContext.hx", "lineNumber" => 244, "className" => "ufront.web.context.HttpContext", "methodName" => "new")), "type" => ufront_log_MessageType::$MLog)));
				}
			}
		}
		if($this->auth === null) {
	...
	public function getRequestUri() {
		...
		if(null === $this->_requestUri) {
			$tmp = $this->request->get_uri();
			$url = ufront_web_url_PartialUrl::parse($tmp);
			{
				$_g = 0;
				$_g1 = $this->urlFilters;
				while($_g < $_g1->length) {
	...
	public function generateUri($uri, $isPhysical = null) {
		...
		while($i >= 0) {
			$tmp = $i--;
			_hx_array_get($this->urlFilters, $tmp)->filterOut($uriOut);
			unset($tmp);
		}
		{
			$tmp = $uriOut->toString();
	...
	public function get_currentUserID() {
		...
		$tmp = null;
		if($this->auth !== null) {
			$tmp1 = $this->auth->get_currentUser();
			$tmp = $tmp1 !== null;
		} else {
			$tmp = false;
		}
	...
	public function get_contentDirectory() {
		...
		if($this->_contentDir === null) {
			$tmp = $this->request->get_scriptDirectory();
			if($tmp !== null) {
				$tmp1 = $this->request->get_scriptDirectory();
				$tmp2 = haxe_io_Path::addTrailingSlash($tmp1);
				$tmp3 = haxe_io_Path::addTrailingSlash($this->_relativeContentDir);
				$this->_contentDir = _hx_string_or_null($tmp2) . _hx_string_or_null($tmp3);
			} else {
	...
	static function createContext($request = null, $response = null, $appInjector = null, $session = null, $auth = null, $urlFilters = null, $relativeContentDir = null) {
		...
		{
			$tmp2 = new ufront_web_context_HttpContext($request, $response, $appInjector, $session, $auth, $urlFilters, $relativeContentDir);
			$GLOBALS['%s']->pop();
			return $tmp2;
		}
		$GLOBALS['%s']->pop();
