...
class minject_InjectorMapping {
	...
	public function toProvider($provider) {
		...
		if($tmp) {
			$tmp1 = $this->toString();
			$tmp2 = "Warning: Injector contains " . _hx_string_or_null($tmp1) . ".\x0AAttempting to overwrite this ";
			$tmp3 = $provider->toString();
			haxe_Log::trace(_hx_string_or_null($tmp2) . _hx_string_or_null(("with mapping for " . _hx_string_or_null($tmp3) . ".\x0AIf you have overwritten this mapping ")) . "intentionally you can use `injector.unmap()` prior to your replacement mapping " . "in order to avoid seeing this message.", _hx_anonymous(array("fileName" => "InjectorMapping.hx", "lineNumber" => 84, "className" => "minject.InjectorMapping", "methodName" => "toProvider")));
		}
	...
	public function toString() {
		...
		$tmp1 = "mapping: [" . _hx_string_or_null($this->type) . "]" . _hx_string_or_null($named) . " mapped to [";
		$tmp2 = Std::string($this->provider);
		{
